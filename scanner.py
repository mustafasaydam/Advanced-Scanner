...
def nuclei_scan(target, template_path="~/nuclei-templates"):
    ...
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    output_file = f"reports/nuclei_results_{timestamp}.json"
...
def zap_scan(target, api_key='your-api-key'):
    ...
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    output_file = f"reports/zap_report_{timestamp}.html"
    ...
    try:
        ...
        # Spider ilerlemesini bekle
        while int(zap.spider.status(scan_id)) < 100:
            print(f"{COLOR_YELLOW}[*] Spider tarama devam ediyor: %{zap.spider.status(scan_id)}{COLOR_END}")
            time.sleep(5)
        ...
        while int(zap.ascan.status(scan_id)) < 100:
            ...
            print(f"{COLOR_YELLOW}[*] Aktif tarama devam ediyor: %{status}{COLOR_END}")
            time.sleep(10)
...
def subdomain_scan(target):
    ...
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    output_file = f"reports/subdomains_{timestamp}.txt"
    ...
    sl_file = f"reports/sublist3r_{timestamp}.txt"
    cmd = f"sublist3r -d {target} -o {sl_file}"
    run_command(cmd)
...
def generate_report(nmap_file, nuclei_file, zap_file, subdomain_file=None):
    ...
    if nuclei_file and os.path.exists(nuclei_file):
        try:
            ...
            print(f"{COLOR_RED}[-] Nuclei sonuçları parse edilemedi: {e}{COLOR_END}")
